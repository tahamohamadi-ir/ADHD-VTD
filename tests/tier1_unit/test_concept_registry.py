...
class TestConceptRegistry:
    ...
    def test_enrich_qir(self):
        ...
        # filters check
        has_depression_filter = any(
            f["column"] == "depression_flag" and f["value"] == 1 for f in qir.filters
        )
        has_female_filter = any(
            f["column"] == "gender" and f["value"] == "Female" for f in qir.filters
        )
