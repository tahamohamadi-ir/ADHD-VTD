...
RESEARCH_DISCLAIMER = "**توجه:** این داده‌ها برای مقاصد پژوهشی است و کاربرد بالینی ندارد."

EMPTY_RESULT_ANSWER = "داده‌ای یافت نشد."
...
def format_answer(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if action == "refuse_privacy":
        return {
            "final_answer": "متاسفانه به دلایل حفظ حریم خصوصی، امکان نمایش اطلاعات فردی وجود ندارد."
        }
    if action == "ask_clarification":
        return {"final_answer": "لطفاً سوال خود را شفاف‌تر بیان کنید تا بتوانم کوئری دقیقی بسازم."}
    if action == "fail_gracefully":
        return {
            "final_answer": "تلاش برای تولید پاسخ مناسب موفقیت‌آمیز نبود. لطفاً سوال خود را تغییر دهید."
        }
    if action == "answer_without_sql":
        return {
            "final_answer": "این یک سوال مشاوره‌ای/تعریفی است. سیستم در حال حاضر فقط تحلیل داده‌های جدولی را پشتیبانی می‌کند."
        }
    if action == "answer_chart_recommendation":
        return {
            "final_answer": "این سوال نیاز به پیشنهاد نمودار دارد. (تحلیل و نمودار پیشنهادی به زودی پشتیبانی می‌شود)"
        }
