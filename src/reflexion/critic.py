...
class SQLCritic:
    ...
    def analyze(self, sql: str, error_msg: str, schema_context: Optional[str] = None) -> str:
        ...
        feedback = []
        feedback.append("The previous SQL query failed.")
        ...
        if taxon.code == "no_such_table":
            feedback.append(
                "Suggestion: Review the schema and ensure you are using the correct table names. Tables available in context should be used."
            )
        elif taxon.code == "no_such_column":
            feedback.append(
                "Suggestion: The column you referenced does not exist in the specified table. Check the schema for correct column names."
            )
        elif taxon.category == "syntax_error":
            feedback.append(
                "Suggestion: Check the SQL syntax, especially quotes, parentheses, and SQLite-specific keywords."
            )
