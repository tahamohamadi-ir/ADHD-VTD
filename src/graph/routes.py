...
def route_pre_generation(
    state: VTDState,
) -> Literal["link_schema", "ask_clarification", "refuse_unsafe_sql", "answer_without_sql"]:
...
def route_after_validation(
    state: VTDState,
) -> Literal["execute_sql", "reflect_on_error", "fail_gracefully"]:
    ...
    if not state.ablation_config.get("reflexion", True) or not state.ablation_config.get(
        "repair", True
    ):
...
def route_after_execution(
    state: VTDState,
) -> Literal["compute_reliability", "reflect_on_error", "fail_gracefully"]:
...
def route_after_reliability(
    state: VTDState,
) -> Literal[
    "format_answer", "reflect_on_error", "fail_gracefully", "ask_clarification", "refuse_unsafe_sql"
]:
    ...
    action = decision.get("effective_action") or decision.get("action")
    if action == "answer":
        return "format_answer"
