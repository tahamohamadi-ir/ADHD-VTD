...
def generate_sql(state: VTDState) -> Dict[str, Any]:
    ...
    # TODO: Load model path from config
    model_path = "models/generation/qwen2.5-coder-7b-instruct-q4_k_m.gguf"
    llm = LocalLLM(model_path=model_path, n_ctx=2048, n_gpu_layers=-1)
...
def validate_sql(state: VTDState) -> Dict[str, Any]:
    ...
    """
    if not state.generated_sql:
        return {"validation_errors": [{"message": "No SQL to validate"}]}
...
def execute_sql(state: VTDState) -> Dict[str, Any]:
    ...
    db_path = "data/db/vtd_health_research_v1.db"
    executor = ReadOnlyExecutor(db_path=db_path)
