...
class SchemaLoader:
    ...
    def load_schema_snapshot(
        self, path: str | Path = DEFAULT_SCHEMA_SNAPSHOT_PATH
    ) -> dict[str, Any]:
    ...
    def load_column_aliases(
        self, path: str | Path = DEFAULT_COLUMN_ALIASES_PATH
    ) -> dict[str, list[str]]:
        return self.load_json(path)

    def load_business_glossary(
        self, path: str | Path = DEFAULT_BUSINESS_GLOSSARY_PATH
    ) -> dict[str, Any]:
        return self.load_json(path)

    def load_metric_definitions(
        self, path: str | Path = DEFAULT_METRIC_DEFINITIONS_PATH
    ) -> dict[str, Any]:
        return self.load_json(path)
